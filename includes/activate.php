...
function AV_YT_activate_plugin() {
    ...
    if( ! $AV_YT_options ) {
        ...
        $opts                           = array(
            ...
            'widget_text'   			    => __( 'Subscribe to our channel', 'av-youtube' ),
...
function AV_YT_plugin_settings_link( $links ) {
    ...
    $settings_link = '<a href="' . $url . '">' . __('Settings', 'av-youtube') . '</a>';
...
function AV_YT_activation_admin_notices() {
    ...
    if (  isset( $AV_YT_options['activation_notice_shown'] )
            ...
            && is_plugin_active('av-youtube-subscribe/index.php') 
        ) {

        $notice_type            = 'success';
        $is_dismissible         = true;
        $message                = sprintf( '%s <a href="' . admin_url( 'admin.php?page=' . AV_YT_PLUGIN_SETTING_PAGE ) . '">%s</a>', __( 'Thank you for installing & activating the <strong>AV YouTube Channel Subscribe</strong> plugin.', 'av-youtube' ), __( 'Go to settings &rsaquo;', 'av-youtube' ) );
